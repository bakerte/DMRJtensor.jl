...
function simpledmrg(psi,mpo;m=20,sweeps=10,cutoff=1E-9)
  ...
  for n = 1:sweeps
    for s = 1:2*(length(psi)-1)
      ...
      outAA,energy = simplelanczos(iL,iR,Lenv,Renv,psi,mpo)

      if cld(length(psi),2) == psi.oc
        println("sweep = $n, site = $(psi.oc), energy = ",energy)
        println()
      end

      U,D,V = svd(outAA,[[1,2],[3,4]],m=m,cutoff=cutoff)

      if j > 0
        psi[iL] = U
        psi[iR] = contract(D,2,V,1)
        Lenv[iL+1] = Lupdate(Lenv[iL],psi[iL],psi[iL],mpo[iL])
      else
        psi[iL] = contract(U,3,D,1)
        psi[iR] = V
        Renv[iR-1] = Rupdate(Renv[iR],psi[iR],psi[iR],mpo[iR])
      end
      psi.oc += j
...
initTensor = [zeros(1,physindsize,1) for i=1:Ns]
for i = 1:Ns
...
mpo = convert2MPO(H,physindsize,Ns)

...
function MPO(terms::Vector{W};reverse::Bool=true,countreduce::intType=100,sweeps::intType=2,compresscheck::Integer=20) where W <: MPOterm
  ...
  quadraticterms = findall(w->length(terms[w]) == 2,1:length(terms))

  OpTypes = Array{Tuple,1}(undef,0) #Array{Tuple{typeof(terms[1][3]),typeof(terms[1][5])},1}(undef,0)
  Optype = Array{intType,1}(undef,length(quadraticterms))
#  OpRanges = Array{Array{intType,2},1}(undef,0)
  for z = 1:length(quadraticterms) #a in quadraticterms
    a = quadraticterms[z]
    w = 0
    while w < length(OpTypes) && OpTypes[w+1] != (terms[a][2],terms[a][4])
      w += 1
    end
    if w == length(OpTypes)
      push!(OpTypes,(terms[a][2],terms[a][4]))
#      push!(OpRanges,zeros(Int64,Ns))
    end    
    Optype[z] = w+1
  end

  maxOps = length(OpTypes) #number of different operators in quadratic terms

  maxrangesOp = zeros(Int64,Ns,maxOps)
  for a in quadraticterms
    i,j = terms[a][3],terms[a][5]
    t = Optype[a]
    maxrangesOp[i,t] = max(abs(j-i),maxrangesOp[i])
  end

  quadmpos = Array{Array{mpotype,2},1}(undef,Ns)
  for w = 1:Ns
    maxbond = 
    quadmpos[w] = zeros(mpotype,physind[w]*maxbond,physind[w]*maxbond)
  end

  for w = 1:length(maxrangesOp)

  end

  
  #=
  Threads.@threads for a in quadraticterms

...
function applyMPO(psi::MPS,H::MPO;m::Integer=0,cutoff::Float64=0.)
  ...
  for i = thissize:-1:2
    ...
    if m == 0
      L,finalpsi[i] = lq(currTens,[[1,2],[3,4]])
      finalpsi[i-1] = contract(newpsi[i-1],(4,5),L,(1,2))
    else
      U,D,finalpsi[i] = svd(currTens,[[1,2],[3,4]],m = m,cutoff=cutoff)
#      finalpsi[i] = reshape!(V,size(D,1),newsize[3],newsize[4],merge=true)
      tempT = U*D#contract(U,2,D,1)  
      finalpsi[i-1] = contract(newpsi[i-1],(4,5),tempT,(1,2))
    end
  end
